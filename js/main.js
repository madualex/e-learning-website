...
function coursePreviewVideo() {
    ...
    if (coursePreviewModal) {
        coursePreviewModal.addEventListener("shown.bs.modal", function() {
            document.querySelector(".js-course-preview-video").play();
            document.querySelector(".js-course-preview-video").currentTime = 0;
        });

        coursePreviewModal.addEventListener("hide.bs.modal", function() {
            document.querySelector(".js-course-preview-video").pause();
        });
    }
}
...
function headerMenu() {
    ...
    menu.addEventListener('click', (event) => {
        ...
        if (target.classList.contains('js-toggle-sub-menu') && window.innerWidth <= menuCollapseBreakpoint) {
            // prevent default achor click behaviour
            ...
            // if menu-item is not expanded...... run below code
